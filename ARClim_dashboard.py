...
def update_graph(lat, lon, season, variable, modo):
    try:
        if modo == 'presente':
            fig = plot_present_future(lat, lon, season, variable)
        else:
            fig = plot_delta_present_future(lat, lon, season, variable)
        buf = io.BytesIO()
        ...
        return html.Div(f"Ocurrió un error: {str(e)}", style={'color': 'red'})
